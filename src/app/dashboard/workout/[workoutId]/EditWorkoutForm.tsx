...
function deriveStartTime(startedAt: Date | null): string {
  if (!startedAt) return "";
  const hours = startedAt.getHours().toString().padStart(2, "0");
  const minutes = startedAt.getMinutes().toString().padStart(2, "0");
  return `${hours}:${minutes}`;
}

function deriveExerciseEntries(
  workout: WorkoutWithExercises
): ExerciseEntry[] {
  return workout.workoutExercises.map((we) => ({
...
export default function EditWorkoutForm({
  ...
  const [date, setDate] = useState<Date>(workout.date);
  const [startTime, setStartTime] = useState(
    deriveStartTime(workout.startedAt)
  );
  const [notes, setNotes] = useState(workout.notes ?? "");
  const [exerciseEntries, setExerciseEntries] = useState<ExerciseEntry[]>(
    deriveExerciseEntries(workout)
  );
